...
\begin{itemize}
 ...
 \begin{tabular}{|c|r|r|r|r|r|r|r|r|r|r|r|r|}
  ...
  Se observa \textit{overfitting} bastante fuerte en algunas de las ejecuciones para los tres algoritmos. Observamos además que las
  ...
  La hibridación memética funciona muy mal en este \textit{dataset}, donde las metaheurísticas programadas tienen un \textit{score}
  5 puntos por debajo de los resultados obtenidos mediante SFS, y no superan a 3NN.
  
  Se observan algunos \textit{outliers} claros en los resultados y tiempos que rondan la mitad de lo que tardaban los genéticos
  sin hibridar. Aún habiendo \textit{outliers} en los resultados, la desviación típica de los porcentajes de clasificación
  obtenidos son menores que en otros \textit{datasets}.
  
  \imagen{../data/memetic/arrhythmia.png}{Tasas de clasificación en Arrhythmia}{wdbcgraph}{0.7}
  
  \subsection{Notas finales}
  
  Los algoritmos meméticos no han resultado obtener tan buenos resultados como se esperaba de ellos, ya que a priori, podían
  proporcionar un buen equilibrio diversidad-convergencia. De hecho, ahí parece haber radicado el problema, puesto que el
  \textit{setting} que hemos empleado para garantizar tal equilibrio podría no haber sido el adecuado.
